...
def softplus(x: Array, beta: float, gamma: float) -> Array:
    return (jax.nn.softplus(x * beta) / beta) ** (1 / gamma)
...
class ShapeCoef(lsl.Var):
    def __init__(self, delta: lsl.Var) -> None:
        def _compute_shape_coef(delta):
            exp_delta = delta
            exp_delta = exp_delta.at[1:, :].set(
                softplus(delta[1:, :], beta=4.0, gamma=1.5)
            )
